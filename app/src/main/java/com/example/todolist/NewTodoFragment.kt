...
class AddNewTask : BottomSheetDialogFragment() {
    ...
    private lateinit var objectsid: String
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (bundle != null) {
            isUpdate = true
            text = bundle.getString("title").toString()
            objectsid = bundle.getString("objectid").toString()
            fragment_newTaskText.setText(text)
            if (text.isNotEmpty()) {
                frangment_newTaskButton.setTextColor(resources.getColor(R.color.wenzi))
            }
        }
        ...
        fragment_newTaskText.addTextChangedListener(object : TextWatcher {
            ...
            override fun onTextChanged(s: CharSequence, start: Int, before: Int, count: Int) {
                if (s.toString() == "") {
                    frangment_newTaskButton?.isEnabled = false
                    frangment_newTaskButton.setTextColor(Color.GRAY)
                } else {
                    frangment_newTaskButton?.isEnabled = true
                    frangment_newTaskButton.setTextColor(resources.getColor(R.color.wenzi))
                }
            }
        ...
        frangment_newTaskButton.setOnClickListener{
            ...
            if (title.isNotEmpty()) {
                if (isUpdate) {
                    Thread{
                        val todo = LCObject.createWithoutData("Todo", objectsid)
